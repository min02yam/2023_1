...
quantile(dt$빈도 ,prob=c(0.05,0.1, 0.25, 0.5, 0.75, 0.95)) 
...
```
인사이트 - 전체, 상위5퍼센트까지 모두  해운대구>기장군>부산진구>강서구>수영구 위주임
...
dt[,freq := sum(빈도), keyby=.(중분류,소분류)][order(-freq)]
...
ggplot() +
 ...
 geom_col() +
  geom_text(aes(x = 범례, y = 수치, label = 수치, vjust = -0.7))+
  scale_x_discrete(limits = c("주차장 부족", "관광정보 안내부족", "관광지간 먼 이동거리",
       "대중교통 불편", "부당요금","불친절","예역 취소",
       "위생 불량", "음식점 불만", "쇼핌시설","장애인시설",
       "의사소통", "기타"))+
 scale_fill_hue(direction = 1) +
